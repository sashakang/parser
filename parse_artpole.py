    ...
    for item in items:
        ...
        try:
            ...
            name = item.find_element(
                By.CLASS_NAME, 'img-cat-lvl1').get_attribute('title')
        ...
        try:
            specs = item.find_element(
                By.CLASS_NAME, 'sostav-item-color-wr').text
        except NoSuchElementException:
            pass
...
def clean_data(df):
    ...
    df.list_price = df.list_price.apply(lambda v:
            0 if v == 0 else int(''.join(re.findall(r'\d+', v))) if pd.notna(v) else 0)

    df.discount = df.discount.apply(lambda v:
            0 if v == 0 else int(re.search(r'\d+', v)[0]) / 100 if pd.notna(v) else 0)
